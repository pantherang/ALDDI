...
import pandas as pd
import os
from DSNDDI.models import *

from torch_geometric.data import Batch
from sklearn import metrics

class Passive_DSNDDI_Trainer:
    ...
    # trainer.train(h_train_iter, t_train_iter, rels_train_iter, b_train_iter, labels_train_iter)
    def train(self, h_train, t_train, rels_train, b_train, labels_train, cur_idx):
        # model.py
        # def train_DSNDDI(h_train, t_train, rels_train, b_train, labels_train, device, train_batch_size, train_num_epoch, num_rels)
        self.model = train_DSNDDI(h_train, t_train, rels_train, b_train, labels_train, num_rels=self.num_rels, train_batch_size=self.train_batch_size, train_num_epoch = self.train_num_epoch, device = self.device)

    def pred(self, h_train, t_train, rels_train, b_train, labels_train, ret_features = False, ret_margins=False):
        preds = []
        device = self.device
        total_feature = []
        min_margins = []
        ...
        for i in tqdm(range(int(np.ceil(len(h_train) / 500.)))): 
            ...
            #pred = self.model((h_batch.to(device), t_batch.to(device), rels_batch.to(device), b_batch.to(device)))
            #preds.append(pred)
            if ret_features == True:
                pred, f = self.model((h_batch.to(device), t_batch.to(device), rels_batch.to(device), b_batch.to(device)), ret_features = True)
                preds.append(pred)
    ...
    # trainer.test(h_test, t_test, rels_test, b_test, labels_test)
    def test(self, h_test, t_test, rels_test, b_test, labels_test):
        device = self.device
        preds = []
