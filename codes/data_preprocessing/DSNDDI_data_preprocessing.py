...
df_drugs_smiles = pd.read_csv('./data/drug_smiles_db.csv')
df_all_pos_ddi = pd.read_csv('./data/ddis_db.csv')
...
def DSNDDI_preprocess_data(head_list_train, tail_list_train, rels_train, labels_train):
    total_len = len(head_list_train)
    ...
    for i in range(total_len):
        h, t, r, l= head_list_train[i], tail_list_train[i], rels_train[i], labels_train[i]
        h_data = __create_graph_data(h)
        t_data = __create_graph_data(t)
        h_graph = drug_to_mol_graph[h]
        t_graph = drug_to_mol_graph[t]
        b_graph = _create_b_graph(get_bipartite_graph(h_graph,t_graph),h_data.x,t_data.x)
    ...
    rels = rels_train
    labels = labels_train
    tri = (h_samples, t_samples, rels, b_samples, labels)

    return tri

# SSI-DDI
def SSIDDI_preprocess_data(head_list_train, tail_list_train, rels_train, labels_train):
    total_len = len(head_list_train)
    rels = []
    labels = []
    h_samples = []
    ...
    rels = rels_train
    labels = labels_train
    tri = (h_samples, t_samples, rels, labels)
    return tri